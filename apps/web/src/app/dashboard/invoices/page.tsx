        ...
        <div className="p-6 max-w-[1600px] mx-auto flex flex-col md:flex-row gap-6 h-[calc(100vh-theme(spacing.16))]">
            ...
            <div className="w-full md:w-1/3 lg:w-1/4 flex flex-col gap-4 overflow-hidden">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Danh sách hóa đơn thanh toán từ Đặt phòng</p>
                </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                    {loading ? (
                        ...
                        bookings.map(booking => (
                            <Card
                                key={booking.id}
                                className={`cursor-pointer transition-colors border-gray-200 ${selectedInvoice?.id === booking.id ? 'bg-gray-100 border-orange-500/50' : 'bg-white hover:bg-gray-50'}`}
                                onClick={() => setSelectedInvoiceId(booking.id)}
                            >
                                <CardContent className="p-4">
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="font-bold text-gray-900 text-sm">HĐ: {booking.code}</div>
                                    ...
                                    <div className="text-sm text-gray-600">Khách: {booking.guest?.name || 'Walk-in'}</div>
                                    <div className="text-xs text-gray-400 mt-1 flex justify-between items-center">
                                        <span>{format(new Date(booking.createdAt), 'dd/MM/yyyy')}</span>
                                        <span className="font-semibold text-gray-700">{booking.totalAmount.toLocaleString('vi-VN')} đ</span>
                                    </div>
                                </CardContent>
                            </Card>
                        ))
                ...
                {selectedInvoice ? (
                    <>
                        <div className="bg-gray-50 border-b border-gray-200 p-4 flex justify-between items-center no-print">
                            <h2 className="text-lg font-bold text-gray-900">Chi tiết Hóa đơn #{selectedInvoice.code}</h2>
                            <div className="flex gap-2">
                                <Button variant="outline" className="border-gray-300 text-gray-600 hover:bg-gray-100 hover:text-blue-700" onClick={handlePrint}>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-white text-zinc-900 print:p-0 print:overflow-visible mx-4 my-4 rounded-lg shadow-inner print:shadow-none print:m-0 id-print-container">
                            ...
                            <div className="grid grid-cols-2 gap-8 mb-8">
                                <div>
                                    <h3 className="text-xs font-bold uppercase text-gray-500 mb-2">Thông tin Khách hàng</h3>
                                    <div className="bg-zinc-50 p-4 rounded-lg border border-zinc-100">
                                        <div className="font-bold text-lg text-zinc-800 mb-1">{selectedInvoice.guest?.name}</div>
                                        <div className="text-sm flex items-center gap-2 text-gray-400 mb-1"><CreditCard className="w-3 h-3" /> CCCD/CMND: {selectedInvoice.guest?.idNumber || 'Không có'}</div>
                                        <div className="text-sm flex items-center gap-2 text-gray-400 mb-1"><Clock className="w-3 h-3" /> SĐT: {selectedInvoice.guest?.phone || 'Không có'}</div>
                                    </div>
                                </div>
                                <div>
                                    <h3 className="text-xs font-bold uppercase text-gray-500 mb-2">Chi tiết Lưu trú</h3>
                            ...
                            <table className="w-full mb-8 text-sm">
                                <thead className="bg-zinc-100 text-gray-400">
                                    <tr>
                                        ...
                                        return (
                                            <tr key={br.id} className="border-b border-zinc-100">
                                                <td className="py-3 px-3">
                                                    ...
                                                    <div className="text-xs text-gray-400">{br.roomType?.name}</div>
                                                </td>
                            ...
                            <table className="w-full mb-8 text-sm">
                                ...
                                <tbody>
                                    {(selectedInvoice?.serviceUsages?.length ?? 0) > 0 ? selectedInvoice.serviceUsages?.map((usage: any) => (
                                        ...
                                        <tr>
                                            <td colSpan={4} className="py-4 px-3 text-center text-gray-400 italic">Không sử dụng dịch vụ bổ sung nào.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                                ...
                                <tbody>
                                    {(selectedInvoice?.payments?.length ?? 0) > 0 ? selectedInvoice.payments?.map((payment: any) => (
                                        <tr key={payment.id} className="border-b border-zinc-100">
                                            <td className="py-3 px-3 text-gray-300">{format(new Date(payment.createdAt), 'dd/MM/yyyy HH:mm')}</td>
                                            <td className="py-3 px-3 text-gray-300 uppercase font-medium">{payment.method}</td>
                                            <td className="py-3 px-3 text-right font-semibold text-emerald-600">{payment.amount.toLocaleString('vi-VN')}</td>
                                        </tr>
                                    )) : (
                                        <tr>
                                            <td colSpan={3} className="py-4 px-3 text-center text-gray-400 italic text-sm">Chưa có giao dịch thanh toán nào được ghi nhận.</td>
                                        </tr>
                                    )}
                                    ...
                                    {selectedInvoice.paidAmount > 0 && (!selectedInvoice.payments || selectedInvoice.payments.length === 0) && (
                                        <tr className="border-b border-zinc-100">
                                            <td className="py-3 px-3 text-gray-300">{format(new Date(selectedInvoice.createdAt), 'dd/MM/yyyy HH:mm')}</td>
                                            <td className="py-3 px-3 text-gray-300 font-medium">Khách trả trước / Cọc</td>
                            ...
                            <div className="mt-16 pt-8 border-t border-zinc-200 text-center text-sm text-gray-400">
