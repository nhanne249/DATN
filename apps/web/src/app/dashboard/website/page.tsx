        ...
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-sm">Cấu hình giao diện và nội dung trang clientpage của khách sạn</p>
                </div>
                <div className="flex items-center gap-3">
                    {clientUrl ? (
                        <a href={clientUrl} target="_blank" rel="noopener noreferrer">
                            <Button variant="outline" className="bg-gray-50 border-gray-300 text-gray-600 hover:bg-gray-100 gap-2">
                                <ExternalLink className="w-4 h-4" /> Xem website
                            </Button>
                        </a>
                    ) : (
                        <Button variant="outline" disabled className="bg-gray-50 border-gray-300 text-gray-400 gap-2">
                    ...
                    <Button
                        ...
                        className={config.isPublished
                            ...
                            : 'bg-gray-50 border-gray-300 text-gray-500 hover:bg-gray-100 gap-2'}
                    >
            ...
            <Tabs defaultValue="info" className="space-y-4">
                <TabsList className="bg-gray-50 border border-gray-200">
                    <TabsTrigger value="info" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-900 gap-2">
                        <Info className="w-4 h-4" /> Thông tin
                    </TabsTrigger>
                    <TabsTrigger value="design" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-900 gap-2">
                        <ImageIcon className="w-4 h-4" /> Giao diện
                    </TabsTrigger>
                    <TabsTrigger value="promotions" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-900 gap-2">
                        <Tag className="w-4 h-4" /> Ưu đãi
                    </TabsTrigger>
                    <TabsTrigger value="seo" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-900 gap-2">
                ...
                <TabsContent value="info">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <Card className="bg-white border-gray-200">
                            <CardHeader><CardTitle className="text-gray-900 text-base">Thông tin cơ bản</CardTitle></CardHeader>
                        ...
                        <Card className="bg-white border-gray-200">
                            <CardHeader><CardTitle className="text-gray-900 text-base">Liên hệ</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
                        ...
                        <Card className="bg-white border-gray-200 lg:col-span-2">
                            <CardHeader><CardTitle className="text-gray-900 text-base">Tiện ích</CardTitle></CardHeader>
                            <CardContent>
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {(config.amenities || []).map((a: string, i: number) => (
                                        <span key={i} className="flex items-center gap-1 bg-gray-100 text-gray-600 px-3 py-1.5 rounded-full text-sm">
                                            {a}
                                ...
                                <div className="flex gap-2">
                                    <input value={newAmenity} onChange={e => setNewAmenity(e.target.value)}
                                        ...
                                        placeholder="Thêm tiện ích..." className="flex-1 bg-gray-50 border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none" />
                                    <Button size="sm" variant="outline" className="bg-gray-50 border-gray-300 text-gray-600" onClick={() => {
                ...
                <TabsContent value="design">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <Card className="bg-white border-gray-200">
                            <CardHeader><CardTitle className="text-gray-900 text-base">Màu sắc</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
                                <div>
                                    <label className="text-gray-500 text-sm mb-2 block">Màu chủ đạo</label>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-gray-600 font-mono text-sm">{config.primaryColor}</span>
                                    </div>
                                </div>
                                <div>
                                    <label className="text-gray-500 text-sm mb-2 block">Màu nhấn</label>
                                    <div className="flex items-center gap-3">
                                        <input type="color" value={config.accentColor} onChange={e => setConfig({ ...config, accentColor: e.target.value })}
                                            className="w-12 h-12 rounded-lg cursor-pointer border-0 bg-transparent" />
                                        <span className="text-gray-600 font-mono text-sm">{config.accentColor}</span>
                                    </div>
                                </div>
                                {/* Preview */}
                                <div className="rounded-xl p-4 space-y-2" style={{ background: config.primaryColor + '15' }}>
                                    <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider">Xem trước màu sắc</p>
                        ...
                        <Card className="bg-white border-gray-200">
                            <CardHeader><CardTitle className="text-gray-900 text-base">Hình ảnh</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
                ...
                <TabsContent value="promotions">
                    <Card className="bg-white border-gray-200">
                        <CardHeader className="flex flex-row items-center justify-between">
                            <CardTitle className="text-gray-900 text-base">Ưu đãi & Khuyến mãi</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {/* Add promo form */}
                            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 space-y-3">
                                <p className="text-gray-500 text-sm font-semibold">Thêm ưu đãi mới</p>
                                <div className="grid grid-cols-2 gap-3">
                                    <input placeholder="Tên ưu đãi *" value={newPromo.title} onChange={e => setNewPromo({ ...newPromo, title: e.target.value })}
                                        className="col-span-2 bg-gray-100 border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none" />
                                    <textarea placeholder="Mô tả ưu đãi" value={newPromo.description} onChange={e => setNewPromo({ ...newPromo, description: e.target.value })} rows={2}
                                        className="col-span-2 bg-gray-100 border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none resize-none" />
                                    <div>
                                        <label className="text-gray-400 text-xs mb-1 block">Giảm giá (%)</label>
                                        <input type="number" placeholder="Ví dụ: 20" value={newPromo.discount} onChange={e => setNewPromo({ ...newPromo, discount: e.target.value })}
                                            className="w-full bg-gray-100 border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none" />
                                    </div>
                                    <div>
                                        <label className="text-gray-400 text-xs mb-1 block">Hạn sử dụng</label>
                            ...
                            <div className="space-y-3">
                                {config.promotions.length === 0 && (
                                    <p className="text-gray-400 text-sm text-center py-4">Chưa có ưu đãi nào. Thêm ưu đãi đầu tiên!</p>
                                )}
                                {config.promotions.map((p: any, i: number) => (
                                    <div key={i} className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex justify-between items-start">
                                        <div>
                                            <p className="text-gray-900 font-semibold">{p.title} {p.discount && <span className="text-green-400 ml-2">-{p.discount}%</span>}</p>
                                            <p className="text-gray-500 text-sm mt-1">{p.description}</p>
                                            {p.validUntil && <p className="text-gray-400 text-xs mt-1">Hết hạn: {p.validUntil}</p>}
                                        </div>
                                        <button onClick={() => setConfig({ ...config, promotions: config.promotions.filter((_: any, idx: number) => idx !== i) })}
                                            className="text-gray-400 hover:text-red-400 ml-4 mt-1">
                ...
                <TabsContent value="seo">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <Card className="bg-white border-gray-200">
                            <CardHeader><CardTitle className="text-gray-900 text-base">Mạng xã hội</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
                        ...
                        <Card className="bg-white border-gray-200">
                            <CardHeader><CardTitle className="text-gray-900 text-base">SEO</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
...
function Field({ label, value, onChange, hint, multiline }: {
    ...
    const cls = "w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-gray-900 text-sm focus:outline-none focus:border-gray-300 placeholder:text-gray-400";
    return (
        <div>
            <label className="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1.5 block">{label}</label>
            {multiline
