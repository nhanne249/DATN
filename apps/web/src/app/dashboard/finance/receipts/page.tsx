...
export default function ReceiptsPage() {
    ...
    const getMethodDisplay = (method: string) => {
        ...
        return <span className="text-gray-500 bg-gray-100 px-2 py-0.5 rounded text-xs font-medium uppercase">{method}</span>;
    };
        ...
        <div className="p-6 max-w-[1600px] mx-auto space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Danh sách những lần nhận tiền vào (tiền cọc, thanh toán)</p>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-zinc-800/50 text-gray-600">
                            {loading ? (
                                <tr>
                                    <td colSpan={9} className="px-4 py-8 text-center text-gray-400">Đang tải biểu dữ liệu phiếu thu...</td>
                                </tr>
                            ) : filteredPayments.length > 0 ? (
                                filteredPayments.map(payment => (
                                    <tr key={payment.id} className="hover:bg-gray-50/30 transition-colors group">
                                        <td className="px-4 py-3">
                                            <div className="font-medium text-gray-900 flex items-center gap-1 cursor-pointer group-hover:text-emerald-400 transition-colors">
                                ...
                                <tr>
                                    <td colSpan={9} className="px-4 py-8 text-center text-gray-400">Không tìm thấy giao dịch nào phù hợp.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
