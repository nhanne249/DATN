    ...
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen text-gray-900">
      <div className="flex justify-between items-center border-b border-gray-200 pb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Cấu hình luồng xử lý và thông báo tự động</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="bg-gray-50 border-gray-200">
          <TabsTrigger value="flows" className="data-[state=active]:bg-gray-100">
            <Zap className="w-4 h-4 mr-2" /> Quy trình tự động
          </TabsTrigger>
          <TabsTrigger value="templates" className="data-[state=active]:bg-gray-100">
            ...
            {isLoadingFlows ? (
              ...
              <Card className="bg-gray-50 border-gray-200 border-dashed">
                <CardContent className="py-20 text-center">
                  ...
                  <p className="text-gray-400">Chưa có quy trình tự động nào được thiết lập.</p>
                </CardContent>
              </Card>
            ) : (
              flows.map(flow => (
                <Card key={flow.id} className="bg-gray-50 border-gray-200 hover:border-gray-300 transition-colors">
                  <CardContent className="p-5 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-full ${flow.isActive ? 'bg-emerald-500/10 text-emerald-500' : 'bg-gray-100 text-gray-400'}`}>
        ...
        <TabsContent value="templates" className="space-y-4">
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoadingTemplates ? (
              <>
                <Skeleton className="h-[150px] w-full bg-gray-50" />
                <Skeleton className="h-[150px] w-full bg-gray-50" />
              </>
            ) : templates.length === 0 ? (
               <Card className="col-span-full bg-gray-50 border-gray-200 border-dashed">
                <CardContent className="py-20 text-center">
                  <Mail className="h-12 w-12 text-gray-300 mx-auto mb-4" />
                  <p className="text-gray-400">Chưa có mẫu email nào.</p>
                </CardContent>
              </Card>
            ) : (
              templates.map(template => (
                <Card key={template.id} className="bg-gray-50 border-gray-200 hover:border-gray-300 transition-colors cursor-pointer">
                  ...
                  <CardContent>
                    ...
                    <div className="mt-4 flex justify-between items-center">
                       <span className="text-[10px] text-gray-400">ID: {template.id.slice(0, 8)}</span>
