...
export class AuditLogInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const details: Record<string, unknown> = {}; // { body: req.body, query: req.query, params: req.params }

    return next.handle().pipe(
      tap({
        next: () => {
          this.auditLogService
            .logAction(userId, action, ipAddress, details)
            .catch((err) => {
        ...
        error: (error: Error) => {
          ...
          this.auditLogService
            .logAction(userId, `${action}_FAILED`, ipAddress, errorDetails)
            .catch((err) => {
