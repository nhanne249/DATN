...
export class DashboardService {
  ...
  async getSummary(propertyId: string, startDate?: Date, endDate?: Date) {
    ...
    const occupancyRate =
      totalRooms > 0
        ? Math.round((bookings.length / (totalRooms * 30)) * 100)
        : 0;
    const revPar =
      totalRooms > 0 ? Math.round(totalRevenue / (totalRooms * 30)) : 0;
    ...
    const chartData = days.map((day) => {
      ...
      return {
        ...
        occupancy:
          totalRooms > 0
            ? Math.round((dayBookings.length / totalRooms) * 100)
            : 0,
      };
    });
    ...
    return {
      ...
      roomStatusTabs: [
        ...
        },
        {
          key: 'checked_in',
          label: 'Đang ở',
          count: bookings.filter((b) => b.status === BookingStatus.CHECKED_IN)
            .length,
        },
        {
          key: 'checked_out',
          label: 'Đã trả',
          count: bookings.filter((b) => b.status === BookingStatus.CHECKED_OUT)
            .length,
        },
        {
          key: 'pending',
          label: 'Chờ xử lý',
          count: bookings.filter((b) => b.status === BookingStatus.PENDING)
            .length,
        },
      ],
      tableData,
